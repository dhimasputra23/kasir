...
class M_barang_rusak extends CI_Model{
	...
	{
		$this->db->select('*, barang_rusak.id as id_barang_rusak');
		$this->db->from('barang_rusak');
		$this->db->join('tbl_barang','tbl_barang.barang_id=barang_rusak.id_barang');
	...
	public function edit_barang()
	{
		...
		$tampung = $this->db->query("select * from barang_rusak where id='$id_barang_rusak'")->row_array();
	...
	{
		$tampung = $this->db->query("select * from barang_rusak   where id='$id'")->row_array();
